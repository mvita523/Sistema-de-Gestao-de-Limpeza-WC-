...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def show_report(self, query, error="", success=False):
        ...
        student_number = (query.get("student_number") or [""])[0]
    ...
    def create_report(self):
        ...
        student_number = form.get("student_number", "").strip()[:80]
        max_location_id = get_max_location_id()
        ...
        with connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    RETURNING id
                    """,
                    (int(location_id), issue_type, description, student_number or None),
                )
        ...
        with connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    """
                    UPDATE reports
                    SET status = 'resolved', resolved_at = COALESCE(resolved_at, NOW()), resolved_by_id = NULL
                    WHERE id = %s
                    """,
                    (report_id,),
                )
    ...
    def report_rows(self, reports, status, location_id):
        ...
        for report in reports:
            ...
            if report.get("student_number"):
                student_info = f'<p class="muted">N.º estudante: {escape(report["student_number"])}</p>'

            resolved_by_html = ""
            if report["status"] == "resolved":
                resolved_by = report.get("resolved_by_name") or "Administrador"
                resolved_by_html = f'<p class="muted">Resolvido por: {escape(resolved_by)}</p>'
